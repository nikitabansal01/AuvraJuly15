...
def validate_weekly_answer(
    ...
    if answer_type not in ANSWER_TYPES:
        raise unprocessable_content(
            "unknown_weekly_answer_type", "Question definition is invalid."
        )
    if set(answer) != {"value"}:
        raise unprocessable_content(
...
def _validate_scale(value: Any, schema: dict[str, Any]) -> None:
    ...
    if type(value) is not int or type(minimum) is not int or type(maximum) is not int:
        raise unprocessable_content(
            "invalid_weekly_answer", "A scale answer must be an integer."
        )
    if value < minimum or value > maximum:
        raise unprocessable_content(
            "invalid_weekly_answer", "Scale answer is outside its range."
        )
...
def _validate_text(value: Any, schema: dict[str, Any]) -> None:
    ...
    if not value.strip() or len(value) > maximum:
        raise unprocessable_content(
            "invalid_weekly_answer", "Text answer violates its definition."
        )
...
def _validate_boolean(value: Any) -> None:
    if type(value) is not bool:
        raise unprocessable_content(
            "invalid_weekly_answer", "Boolean answer is invalid."
        )
...
def _validate_multi_select(value: Any, schema: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise unprocessable_content(
            "invalid_weekly_answer", "Multi-select answer is invalid."
        )
    if not value or len(value) > maximum or len(value) != len(set(value)):
        raise unprocessable_content(
            "invalid_weekly_answer", "Multi-select values are invalid."
        )
