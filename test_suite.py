...
This file is meant to be run as a script, not imported as a pytest test module.
Pytest collects `test_*.py` and `*_test.py` by default; without guarding the
entrypoint this would crash pytest during collection.
"""
...
def main() -> int:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            test(f"Fallback '{fallback}' in model list", bool(fallback) and fallback in models)
    except Exception as e:
    ...
    if "passed" in result.stdout:
        ...
        test("Pytest", False, (result.stdout + result.stderr)[:200])
