#!/usr/bin/env python3
"""AUVRA Comprehensive Test Suite - Synchronous Version"""
import os
import sys
...
test("GROQ_FALLBACK_MODEL=openai/gpt-oss-120b", os.getenv("GROQ_FALLBACK_MODEL") == "openai/gpt-oss-120b")
...
try:
    action = ActionItemModel(
        title="Test Food", category="food", time_slot="morning",
        specific_action="Test", purpose="Test", target_hormone="cortisol",
        hormone_persona_intro="Test", image_prompt="test",
        research_studies=[], variants=[],
        food_items=["oats"], food_amounts=["1 cup"],
    ...
    test("Valid FOOD action", False, str(e))

try:
    action = ActionItemModel(
        title="Test", category="food", time_slot="lunch",
        specific_action="Test", purpose="Test", target_hormone="cortisol",
        hormone_persona_intro="Test", image_prompt="test",
        research_studies=[], variants=[],
...
# TEST 4: API Keys work (sync check)
...
try:
    ...
    if r.status_code == 200:
        ...
        test(f"Fallback '{fallback}' in model list", fallback in models)
except Exception as e:
    test("Groq API", False, str(e))

# TEST 5: Groq gpt-oss-120b
...
try:
    ...
    test("gpt-oss-120b responds", False, str(e))
...
print("-"*50)
import subprocess
result = subprocess.run(["pytest", "tests/", "-q", "--tb=no"], capture_output=True, text=True, timeout=30)
if "passed" in result.stdout:
    import re
    match = re.search(r"(\d+) passed", result.stdout)
    if match:
        test(f"Pytest ({match.group(1)} tests passed)", True)
else:
    test("Pytest", False, result.stdout[:100])
